...
export default function Dashboard({ initialCases, sourceNames, connected }: { initialCases: Case[]; sourceNames: string[]; connected: boolean }) {
  ...
  return <main>
    ...
    <section className="content">
      ...
      {view === "review" && <ReviewQueue cases={initialCases} onOpen={setActiveCase} approvedIds={approvedIds} onApprove={(id) => setApprovedIds(current => [...current, id])} />}
...
function Sources({ names }: { names: string[] }) { return <div className="admin-page"><p className="eyebrow">REDAKTION</p><h1>Quellen</h1><p>Öffentliche Fachquellen werden täglich um 06:00 Uhr geprüft.</p><div className="source-list">{names.length ? names.map((name, i) => <div key={name}><b>{name.slice(0, 1)}</b><span><strong>{name}</strong><small>{i < 5 ? "Letzter Abruf: heute, 06:00 · Aktiv" : "Letzter Abruf: gestern · Aktiv"}</small></span><Badge tone="new">Aktiv</Badge><button className="outline">Bearbeiten</button></div>) : <div><span>Noch keine Quellen angelegt.</span></div>}</div><button className="primary add-source">+ Quelle hinzufügen</button></div>; }
