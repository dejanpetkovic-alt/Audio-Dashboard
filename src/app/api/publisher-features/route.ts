...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase.from("publisher_feature_observations").insert({ publisher_source_id: sourceId, observed_feature: observedFeature, product_feature_id: typeof body.productFeatureId === "string" && body.productFeatureId ? body.productFeatureId : null, trend_signal_id: typeof body.originTrendId === "string" && body.originTrendId ? body.originTrendId : null, platforms, status: statusMap[String(body.status)], evidence_quality: qualityMap[String(body.evidenceQuality)], evidence_url: evidenceUrl, evidence_label: evidenceLabel, notes: typeof body.notes === "string" ? body.notes.trim() : "" }).select("id").single();
